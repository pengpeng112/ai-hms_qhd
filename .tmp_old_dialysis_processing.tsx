             ...
             <div className="bg-white shadow-2xl w-[210mm] min-h-[297mm] p-[10mm] relative print:shadow-none print:w-full print:h-auto print:p-0 print:mx-0 print:my-0">
                      ...
                      <div className="flex border-b border-gray-400">
                          ...
                          <div className="flex-1 p-2 text-xs">
                              <div className="grid grid-cols-4 gap-2 mb-2">
                                  ...
                                  <div>{t('print.temp')}: 36.5 ‚Ñ</div>
                              </div>
                              <div className="grid grid-cols-2 gap-2">
                                  <div>{t('print.symptoms')}: Êó†ÁâπÊÆä‰∏çÈÄÇÔºåÁ≤æÁ•ûÂ∞öÂèØ</div>
                                  <div>{t('print.fistula')}: ÈúáÈ¢§ËâØÂ•ΩÔºåÁöÆËÇ§ÂÆåÊï</div>
                              </div>
                          </div>
                      </div>
                       <div className="flex">
                          ...
                          <div className="flex-1 p-2 text-xs">
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                  <div>{t('print.anticoagulant')}: ‰ΩéÂàÜÂ≠êËÇùÁ¥?2500iu</div>
                      ...
                      <div className="flex border-b border-gray-400">
                          ...
                          <div className="flex-1 p-2 text-xs">
                              <div className="grid grid-cols-4 gap-2 mb-2">
                                  ...
                                  <div>{t('print.temp')}: 36.6 ‚Ñ</div>
                              </div>
                              <div>{t('print.punctureSite')}: Ê≠¢Ë°ÄËâØÂ•ΩÔºåÊó†Ê∏óË°ÄÔºåÊó†Ë°ÄËÇø„Ä</div>
                          </div>
                      </div>
                       <div className="flex">
                          ...
                          <div className="flex-1 p-2 text-xs min-h-[60px]">
                              ÊÇ£ËÄÖ‰ªäÊó•ÈÄèÊûêËøáÁ®ãÈ°∫Âà©ÔºåÊó†ÁâπÊÆä‰∏çÈÄÇ‰∏ªËØâ„ÄÇÁîüÂëΩ‰ΩìÂæÅÂπ≥Á®≥ÔºåÂÜÖÁòòÁ©øÂà∫ÁÇπÊó†Ê∏óË°ÄÔºåÈÄèÊûêÁªìÊùüÂêéÂéãËø´Ê≠¢Ë°ÄÈ°∫Âà©„ÄÇÊ≤ªÁñóÊïàÊûúËØÑ‰ª∑ÔºöÁ®≥ÂÆö„Ä?                          </div>
                      </div>
                  </div>
                  ...
                  <div className="flex justify-between text-sm mt-8 px-8">
                      <div>
                          ...
                          <span className="border-b border-gray-800 w-32 inline-block text-center font-script text-lg">ÁéãÂåªÁî</span>
                      </div>
                      <div>
...
const DialysisProcessing: React.FC<DialysisProcessingProps> = ({ initialPatientId }) => {
  ...
  useEffect(() => {
    restApi.getPatientList({ page: 1, pageSize: 200 })
      .then(res => {
        ...
        setPatients(list);
        // Êï∞ÊçÆÂä†ËΩΩÂêéËÆæÁΩÆÈªòËÆ§ÈÄâ‰∏≠
        if (!initialPatientId && list.length > 0) {
          const active = list.find(p => p.status === 'Õ∏Œˆ÷–');
          setSelectedPatientId(active?.id || list[0]?.id || null);
        }
      })
      .catch(err => console.error('Âä†ËΩΩÊÇ£ËÄÖÊï∞ÊçÆÂ§±Ë¥?', err));
  }, [initialPatientId]);
  ...
  const renderNurseOptions = () => {
    if (nurses.length === 0) {
      return <option value="">--ËØ∑ÈÄâÊã©--</option>;
    }
      ...
      .then((users) => {
        const nurseList = users
          .filter((user) => {
            ...
            return role.includes('ª§') || role.toLowerCase().includes('nurse');
          })
          .map((user) => ({
            id: String(user.id),
  ...
  const [materials] = useState([
      { id: 1, name: '»Ò’Î-16G', category: '¥©¥Ã’Î', count: 2, code: '', brand: 'NIPRO', spec: '', note: '' },
      { id: 2, name: 'SD-15HF', category: 'Õ∏Œˆ∆˜', count: 1, code: '', brand: 'SWSµ¬¿≥∏£', spec: '', note: '' },
      { id: 3, name: 'ƒ⁄¸«¯', category: 'ª§¿Ì«¯', count: 1, code: '1102011534', brand: '', spec: '', note: '' },
      { id: 4, name: '10MLÊ≥®Â∞ÑÂô?10ML', category: 'ÂÖ∂‰ªñ', count: 1, code: '', brand: '', spec: '10ML', note: '' },
      { id: 5, name: 'JRHLL-025', category: 'Ë°ÄË∑ØÁÆ°', count: 1, code: '', brand: '', spec: '', note: '' },
  ]);
                    ...
                    <div className="flex gap-4 text-sm text-gray-600">
                        ...
                        <div className="flex flex-col">
                         ...
                         <div className="flex flex-col">
                            ...
                            <span className="font-medium">3Âπ?‰∏™Êúà</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="p-6 max-w-[1600px] mx-auto animate-fade-in space-y-6">
                        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-8">
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-x-12 gap-y-6 mb-6">
                                ...
                                <PreAssessInput label={t('pre.temp')} suffix="°Ê" required defaultValue="36.5"/>
                            </div>

                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-x-12 gap-y-6 mb-6">
                                <PreAssessInput label={t('pre.hr')} suffix="¥Œ/∑÷" required defaultValue="64"/>
                                <PreAssessInput label={t('pre.resp')} suffix="¥Œ/∑÷" required defaultValue="16"/>
                                <div className="flex items-center">
                                ...
                                <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-8">
                                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-y-8 gap-x-8">
                                        ...
                                        <div className="col-span-2 md:col-span-3 lg:col-span-6 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-y-8 gap-x-8 border-t border-gray-100 pt-6">
                                            <InfoField label={t('prescription.firstDoseName')} value="ÈÇ£Â±àËÇùÁ¥†ÈíôÊ≥®Â∞ÑÊ∂≤(ÁôæÂäõËà?-615axiu/ml" className="col-span-2" />
                                        ...
                                        <div className="col-span-2 md:col-span-3 lg:col-span-6 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-y-8 gap-x-8 border-t border-gray-100 pt-6">
                                            <InfoField label={t('prescription.vascularAccess')} value="AVF-‰∏äËáÇ" />
                                            <InfoField label={t('prescription.dialysate')} value="A“∫+B“∫" />
                                            ...
                                            <tbody className="divide-y divide-gray-100">
                                                <tr className="hover:bg-red-50/10">
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <span className="text-red-600 font-medium">Ë∂ÖÊª§Èá? Áî±„Ä?„ÄëL Ë∞ÉÊï¥‰∏∫„Ä?.7„ÄëL;</span>
                                                    </td>
                                    ...
                                    <div className="flex items-center space-x-2">
                                        ...
                                        <div className="relative w-48">
                                            <select className="w-full h-8 border rounded text-sm px-2 bg-white appearance-none outline-none">
                                                <option>ÈÇ£Â±àËÇùÁ¥†ÈíôÊ≥®Â∞?..</option>
                                            </select>
                                            <ChevronDown size={14} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none"/>
                                        </div>
                                    </div>
                                ...
                                <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl overflow-hidden animate-scale-in flex flex-col max-h-[95vh]">
                                    ...
                                    <div className="p-8 overflow-y-auto bg-blue-50/10">
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-6">
                                            ...
                                            <MonitorInput label={t('monitor.hr')} suffix="¥Œ/∑÷" defaultValue="65"/>
                                            <MonitorInput label={t('monitor.temp')} suffix="°Ê"/>

                                            <MonitorInput label={t('monitor.resp')} suffix="¥Œ/∑÷" required defaultValue="16"/>
                                            ...
                                            <MonitorInput label={t('monitor.machineTemp')} suffix="°Ê" required defaultValue="35.8"/>
                                            ...
                                            <MonitorInput label={t('monitor.arterialTemp')} suffix="°Ê" defaultValue="0"/>
                        ...
                        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-8">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-6 mb-8">
                                ...
                                <FormField label={t('post.actualDuration')}>
                                    <input type="text" defaultValue="3–° ±30∑÷" disabled className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-500"/>
                                </FormField>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-6 mb-8">
                                ...
                                <FormField label={t('post.postResp')} required>
                                    <div className="relative">
                                        ...
                                        <span className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 text-xs">Ê¨?Âà</span>
                                    </div>
                                </FormField>
