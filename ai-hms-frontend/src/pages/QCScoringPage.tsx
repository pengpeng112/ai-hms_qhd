...
import { getQCDoctors, getQCDoctorDetail, QC_ITEM_LABELS, QC_ITEM_ORDER, type QCDoctorScore, type QCPatientRow } from '@/services/qcApi'
  ...
  return (
    <div className="h-full overflow-y-auto bg-slate-50 p-6">
        ...
        <table className="w-full text-sm whitespace-nowrap">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200 text-slate-500">
              ...
              {QC_ITEM_ORDER.map((k) => (
                <th key={k} className="py-2.5 px-2 text-right font-bold text-[12px]">{QC_ITEM_LABELS[k]}</th>
              ))}
          ...
          <tbody>
            {doctors.map((d) => (
              <tr key={d.doctorId} className="border-b border-slate-50 hover:bg-sky-50/40 cursor-pointer" onClick={() => openDetail(d.doctorId)}>
                <td className="py-2.5 px-3 font-black text-slate-700">医生 #{d.doctorId}</td>
      ...
      <Modal open={detailOpen} onCancel={() => setDetailOpen(false)} footer={null} width={860}
        title={detail ? `医生 #${detail.doctor?.doctorId} · 病人赋分下钻（${monthStr}）` : ''}>
        {detail && (
            ...
            <table className="w-full text-sm whitespace-nowrap">
              <thead>
                <tr className="bg-slate-50 text-slate-500 sticky top-0">
                  ...
                  {QC_ITEM_ORDER.map((k) => <th key={k} className="py-2 px-1.5 text-right font-bold text-[11px]">{QC_ITEM_LABELS[k]}</th>)}
                </tr>
              </thead>
              <tbody>
