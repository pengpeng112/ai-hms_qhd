...
import { CheckCircle2, FileText, NotebookPen, ShieldCheck, TrendingUp, Waves } from 'lucide-react'
...
export default function DialysisSummary({ patient, treatment, treatmentLoading = false }: Props) {
  ...
  const autoSummary = useMemo(() => {
    return `患者于 ${formatDateTime(treatment?.startTime)} 开始 ${treatment?.treatmentType || patient.treatmentPlan || '--'} 治疗，透前体重 ${toText(treatment?.beforeSigns?.weight)}kg，干体重 ${toText(patient.dryWeight)}kg，治疗过程中生命体征${monitoringRows.length > 0 ? '已有监测记录' : '无完整监测趋势'}，实际超滤量为 ${toText(treatment?.afterSigns?.realUfVolume)}L。下机观察凝血分级为 未记录，血管通路状态：未记录，并发症：${treatment?.complications || '无'}。最新备注：${getLatestSymptom(treatment)}。`
    ...
    <div className="space-y-5 pb-8">
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-3">
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="space-y-4">
            <MetricTile label="凝血分级评估" value="未记录" tone="border-indigo-100 bg-indigo-50" />
      ...
      <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          ...
          <button type="button" disabled title="功能待后端接口就绪" className="rounded-lg bg-slate-100 px-5 py-2 text-sm font-bold text-slate-400">保存小结</button>
        </div>
        <div className="grid grid-cols-1 gap-5 p-6 xl:grid-cols-2">
          <label className="block"><span className="mb-3 block text-sm font-bold text-blue-700">医生小结</span><textarea rows={8} readOnly value={treatment?.doctorSummary || ''} placeholder="记录医生对本次透析治疗过程、并发症和后续处理意见的总结。" className="w-full resize-none rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-700 outline-none" /></label>
          <label className="block"><span className="mb-3 block text-sm font-bold text-emerald-700">治疗/护理小结</span><textarea rows={8} readOnly value={treatment?.treatmentSummary || treatment?.notes || ''} placeholder="记录护士对上下机、监测、通路、凝血和患者宣教执行情况的总结。" className="w-full resize-none rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-700 outline-none" /></label>
        </div>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-3">
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm"><h3 className="mb-4 text-sm font-black text-slate-800">凝血与通路摘要</h3><div className="space-y-3 text-sm font-semibold text-slate-600"><div>透析器：未记录</div><div>A端：未记录；V端：未记录</div><div>血管通路：未记录</div><div>并发症：{treatment?.complications || '无'}</div></div></section>
      </div>
