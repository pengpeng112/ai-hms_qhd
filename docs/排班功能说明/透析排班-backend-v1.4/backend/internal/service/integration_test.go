...
// 基础模式 HFD:生成的常规次为 HFD,且落在 HD 机(决策 24/25)。
func TestBaseModeHFD(t *testing.T) {
	g, tenant := setup(t)
	start := futureMonday()
	// 把 1001 基础模式改为 HFD(模板项)
...
func TestWeeklyCountValidation(t *testing.T) {
	...
	bad := &model.PatientProfile{PatientId: 4001, ZoneTag: sched.ZoneA,
		WeeklyCount: 2, FreqPattern: sched.FreqMonWedFri} // 说2次却选一三五(3天)
	if _, err := UpsertProfile(g, tenant, bad); err == nil {
...
// 上机闸门:院感未出拒绝→豁免后可上机→下机;非当日拒绝(决策 26)。
func TestStartTreatmentGates(t *testing.T) {
	g, tenant := setup(t)
	var aw model.Ward
	g.Where(`"TenantId"=? AND "ZoneType"=?`, tenant, sched.ZoneA).First(&aw)
	var mor model.Shift
	g.Where(`"TenantId"=? AND "ShiftCode"=?`, tenant, "MORNING").First(&mor)
	var m model.Machine
	g.Where(`"TenantId"=? AND "MachineType"=?`, tenant, sched.MachineHD).First(&m)
	today := dayStart(time.Now())

	mk := func(pid int64, d time.Time) *model.PatientShift {
		s := &model.PatientShift{BaseModel: model.BaseModel{TenantId: tenant}, PatientId: pid,
			ScheduleDate: d, ShiftId: &mor.Id, WardId: aw.Id, MachineId: &m.Id,
			Status: sched.StatusConfirmed, DialysisMode: sched.ModeHD, SourceType: sched.SourceRegular, RecordForm: sched.RecordFormRegular}
	...
	// 1001 院感未出(seed 默认 unknown)→ 上机被拒
	s1 := mk(1001, today)
	if err := StartTreatment(g, tenant, 1, s1.Id); err != ErrInfectionUnconfirmed {
		t.Fatalf("院感未出应拒绝上机,得 %v", err)
	}
	// 护士长豁免后 → 可上机 → 透析中
	if err := WaiveInfection(g, tenant, 1, 1001); err != nil {
		t.Fatal(err)
	}
	if err := StartTreatment(g, tenant, 1, s1.Id); err != nil {
		t.Fatalf("豁免后应可上机: %v", err)
	}
	var got model.PatientShift
	g.Where(`"Id"=?`, s1.Id).First(&got)
	if got.Status != sched.StatusInDialysis || got.TreatmentStartAt == nil {
		t.Errorf("上机后应为透析中且有上机时间,得 status=%d", got.Status)
	}
	// 下机 → 完成
	if err := CompleteTreatment(g, tenant, 1, s1.Id); err != nil {
		t.Fatal(err)
	}
	g.Where(`"Id"=?`, s1.Id).First(&got)
	if got.Status != sched.StatusCompleted || got.TreatmentEndAt == nil {
		t.Errorf("下机后应为已完成,得 status=%d", got.Status)
	}
	// 非当日(明天)→ 拒绝
	_ = SetInfectionStatus(g, tenant, 1002, sched.InfectionNegative)
	s2 := mk(1002, today.AddDate(0, 0, 1))
	if err := StartTreatment(g, tenant, 1, s2.Id); err != ErrNotToday {
		t.Fatalf("非当日应拒绝上机,得 %v", err)
	}
}
