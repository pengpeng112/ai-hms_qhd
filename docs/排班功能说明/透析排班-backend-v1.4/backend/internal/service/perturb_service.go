...
// parseIdCsv 解析逗号分隔的 ID 串(如 "1,3,5")为 []int64,忽略空白/非法项。
...
// SetHoliday 设置假日并处理受影响排班(决策 19):
// holidayMode=10 全院停诊(当天非透析日,全部排班取消);
// holidayMode=20 值班模式(当天仍透析,仅 openWardIds 开放,未开放区的排班取消)。
// 受影响病人就近(前后 7 天、且该区当天开放)给"挪班建议"入冲突队列;系统不自动改定。
func SetHoliday(g *gorm.DB, tenant int64, date time.Time, holidayMode int16, openWardIds string) (*HolidayResult, error) {
	...
	var affected []*model.PatientShift
	if err := q.Find(&affected).Error; err != nil {
		return nil, err
	}
...
func nearestHolidaySlot(board *sched.Board, wardID, shiftID int64, mode string, d time.Time) (time.Time, *model.Machine) {
	for off := 1; off <= 7; off++ {
		for _, sign := range []int{1, -1} {
			...
			if !board.IsDialysisDay(cand) || !board.WardOpenOn(cand, wardID) {
				continue
			}
