...
func (s *Server) Register(r *gin.Engine) {
	...
	v1.Use(tenantMiddleware())
	{
		...
		v1.GET("/schedule/quality", s.quality)
		...
		v1.POST("/shifts/:id/start", guard(RoleNurse, RoleChargeNurse, RoleHeadNurse), s.startTreatment)
		v1.POST("/shifts/:id/complete", guard(RoleNurse, RoleChargeNurse, RoleHeadNurse), s.completeTreatment)
	...
	s.registerAdmin(v1) // 资源/病人/骨架/模板维护 + 冲突处理(P1)
}
...
// startTreatment POST /shifts/:id/start —— 上机(院感/当日闸门)。
...
// completeTreatment POST /shifts/:id/complete —— 下机(完成)。
