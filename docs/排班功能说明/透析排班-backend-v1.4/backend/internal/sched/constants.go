...
const (
	ModeHD   = "HD"   // 血液透析 → HD 机
	ModeHFD  = "HFD"  // 高通量血液透析 → HD 机(换高通量透析器)
	ModeHP   = "HP"   // 血液灌流 → HD 机(临时附加)
	ModeHDF  = "HDF"  // 血液透析滤过 → HDF 机
	ModeHF   = "HF"   // 血液滤过 → HDF 机
	ModeCRRT = "CRRT" // 连续性肾替代 → CRRT 机
)

// ── 病人在透状态(决策 27)──
const (
	PatientActive     int16 = 10 // 在透
	PatientDischarged int16 = 20 // 已出组
)

// ── 院感状态(决策 26)──
const (
	InfectionNegative = "negative" // 阴性
	InfectionPositive = "positive" // 阳性
	InfectionUnknown  = "unknown"  // 未出/无指标
)
...
// machineCapability 机型能力包含关系(规范 §1.2,决策 24),代码常量,不入库。
// HFD/HP 与 HD 同机;HF 与 HDF 同机;HDF 机向下兼容 HD 类。
var machineCapability = map[string]map[string]bool{
	MachineHD:   {ModeHD: true, ModeHFD: true, ModeHP: true},
	MachineHDF:  {ModeHD: true, ModeHFD: true, ModeHP: true, ModeHDF: true, ModeHF: true},
	MachineCRRT: {ModeCRRT: true},
}

// RequiresHdfMachine 该模式是否必须使用 HDF 机(决策 24):仅 HDF、HF。
// 其余 HD 类(HD/HFD/HP)优先 HD 机、满则溢出 HDF 机。
func RequiresHdfMachine(mode string) bool {
	return mode == ModeHDF || mode == ModeHF
}
...
// ModeMachineTypes 返回某治疗模式可用的机型(按优先级:HD 类优先 HD 机、满则溢出 HDF 机)。
func ModeMachineTypes(mode string) []string {
	switch mode {
	case ModeHD, ModeHFD, ModeHP:
