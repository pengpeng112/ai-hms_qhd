...
// DecideMode 判定某次透析的治疗模式(决策 1:HDF 是替换)。
// 仅当 (启用HDF) 且 (本日==HDF日) 且 (本周奇偶==该病人HDF所属周奇偶) 时本次为 HDF,否则 HD。
// hdfWeekday 用 ISO(周一=1..周六=6);hdfParity 取值 0/1,与 WeekParity 对齐。
func DecideMode(anchor time.Time, hdfEnabled bool, hdfWeekday, hdfParity *int16, d time.Time) string {
