...
func (s *Server) Register(r *gin.Engine) {
	...
	v1.Use(tenantMiddleware())
	{
		...
		// 生成 / 演示数据(管理类)
		v1.POST("/schedule/generate", s.generate)
...
// X-Tenant-Id 缺省 1;X-Role 缺省空(空=超级用户,放行所有,便于演示/联调)。
...
// guard 路由守卫:校验当前角色是否在允许集合内。空角色(未传 X-Role)视为超级用户放行。
func guard(roles ...string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		c.AbortWithStatusJSON(http.StatusForbidden, gin.H{"error": "权限不足:该操作需要 " + joinRoles(roles)})
	}
}
