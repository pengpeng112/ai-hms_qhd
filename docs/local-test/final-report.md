...
1. **并发测试**：补充医嘱/模板应用/库存扣减的并发场景测试
2. **数据清理**：清理 `TEST_AI_HMS_%` 测试数据
3. **npm audit fix**：前端 13 个安全漏洞修复
...
2. **P1 问题全部修复**：14 个 P1 问题已通过代码修复 + go test 验证
3. **核心流程通过**：
...
| 项目 | 优先级 | 状态 |
|------|--------|------|
| 并发场景测试（医嘱/模板/库存） | 高 | 未执行 |
| `TEST_AI_HMS_%` 测试数据清理 | 高 | **已完成**（提交 349802e 后已清理） |
| `npm audit fix` 前端安全漏洞修复 | 中 | 未执行 |
