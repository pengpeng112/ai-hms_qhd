...
| 九 | 统计看板 | 通过 | 0 | 0 | 0 | 0 | Dashboard 类型编码已修复 |
| 十 | 安全/并发 | 部分通过 | 0 | 0 | 0 | 0 | 安全测试通过，并发测试未执行 |

| 合计 | | | **0** | **14** | **3** | **0** | P1全部修复，P2全部澄清/修复 |
...
- 血管通路 CRUD 正常（Note 字段使用 `notes` JSON key 即可正常落库）
...
| Register_VascularAccess | CR | 血管通路新增→落库（`notes` 字段正确映射到 Note 列） |
...
> 执行时间：2026-06-07 18:35，提交 `353be01` 后。

| 表 | 清理前 (`TEST_AI_HMS_%` / `REGRESSION_%`) | 清理后 |
|----|----|----|
| Identity_Users | 4 | **0** |
| Identity_Roles | 3 | **0** |
| Organ_Employee | 4 | **0** |
| Identity_UserRoles | 4 | **0** |
| Register_VascularAccess | ~5 | **0** |
| Plan_PatientPlan | ~3 | **0** |
| Order_PatientOrder | ~2 | **0** |
| Order_PatientDayOrder | ~2 | **0** |
| Schedule_Ward | 1 | **0** |
| Schedule_Bed | 1 | **0** |
| Schedule_Shift | 1 | **0** |
| Schedule_WardExt | 1 | **0** |
| Schedule_BedMachineExt | 1 | **0** |
| Schedule_PatientProfile | 1 | **0** |
| Schedule_TenantSetting | 1 | **0** |
| Schedule_ScheduleTemplate | 1 | **0** |
| Schedule_ScheduleTemplateItem | 1 | **0** |
| Schedule_PatientShift | ~2 | **0** |
| Schedule_PatientShiftExt | ~2 | **0** |
| Treatment_Treatment | ~5 | **0** |
| Treatment_BeforeSigns | ~4 | **0** |
| Treatment_AfterSigns | ~3 | **0** |
| Treatment_DuringParam | ~4 | **0** |
| Auxiliary_EquipmentDisinfection | ~1 | **0** |
| Auxiliary_EquipmentInfomation | ~2 | **0** |
...
| ISSUE-006 | P2 | 患者 | 血管通路 Note 未落库 | 已澄清（字段名 notes/note 输入错误） |
...
| ISSUE-017 | P2 | 统计 | Dashboard 类型编码错误 | 已修复 |

> 共 17 个问题：P1=14（全部已修复），P2=3（全部已修复/已澄清），P0=0。

## 9. 阻断项

### 无 P0 阻断项

所有 P1 问题已全部修复，P2 问题已全部修复或澄清。核心流程均可正常完成。

### 正式扩大试运行前需完成

1. **并发测试**：补充医嘱/模板应用/库存扣减的并发场景测试（当前未执行）
2. **npm audit fix**：前端 13 个安全漏洞修复

> 测试数据清理已在提交 `353be01` 后完成，25 张表全部清零。

## 10. 是否建议进入科室试运行

### 结论：**建议进入受控小范围科室试运行**

> **重要前提**：正式扩大试运行（全科室、全量患者）前，必须补充并发测试（详见第 5 节和第 9 节）。

### 原因

1. **P0 问题 = 0**：无数据错乱、主从不一致、越权、事务不一致等严重问题
2. **P1 问题全部修复**：14 个 P1 问题已通过代码修复 + go test 回归验证
3. **核心流程通过**：
   - 患者管理 → 医嘱 CRUD → 排班配置 → 模板应用 → 透析执行闭环 **全部可完成**
4. **安全合规通过**：SQL注入/XSS/越权防御有效，密码不泄露，账户锁定检测生效
5. **数据一致性验证通过**：主从表关联、TenantId 隔离、CreatorId 记录正确
6. **测试数据已清理**：25 张表 TEST_AI_HMS_% 和 REGRESSION_% 数据全部清零

### 当前未完成项

| 项目 | 优先级 | 说明 |
|------|--------|------|
| 并发场景测试 | 高 | 医嘱/模板/库存并发 — 扩大试运行前必须完成 |
| npm audit fix | 中 | 13 个前端安全漏洞 |
...
- 试运行期间老透析系统保持正常运行，不可停机或降级
- 新系统仅用于**受控小范围**（如 A 区 1~2 个护士 + 1~2 名医生），其余科室仍使用老系统
- 新老系统数据独立存储，互不影响
...
- 每日下班前由指定护士对账当日透析记录：
  - 新系统排班数 vs 老系统排班数
  - 新系统医嘱数 vs 老系统医嘱数
  - 新系统治疗记录数 vs 老系统治疗记录数
- 对账差异 > 0 时立即上报，暂停新系统使用直至查明原因
- 对账工具：使用阶段二~七验证过的 SQL（见各阶段报告）交叉比对

### 11.3 异常回退机制

- 新系统发现任何 P0/P1 问题时，立即切换回老系统操作
- 切换步骤：通知护士站 → 停用新系统患者排班 → 所有操作回老系统 → 记录问题
- 新系统发现 P2 问题时，记录问题但不中断使用，当天下班后统一评估
- 回退后当日已通过新系统录入的数据按老系统重新补录

### 11.4 护士/医生确认机制

- **排班确认**：护士长每日确认排班后，医生签字确认方可执行
- **医嘱确认**：医生开具医嘱后需二次确认，护士执行前需核对
- **透析执行确认**：
  - 透前评估 → 护士确认体征
  - 首次核对 → 护士+医生双签
  - 透后评估 → 护士确认
  - 完成透析 → 护士长确认
- **耗材扣减确认**：耗材使用需护士确认并签字，库存管理员复核
- 所有确认操作在系统中留有电子签名记录（CreatorId/OperatorId），确保可追溯

### 11.5 试运行终止条件

出现以下任一情况时，立即终止试运行并全部切换回老系统：
- 出现任何 P0 问题（数据错乱、主从不一致、越权、事务不一致）
- 同一 P1 问题累计出现 3 次以上
- 对账差异连续 2 天 > 0
- 医生或护士长提出书面终止申请
