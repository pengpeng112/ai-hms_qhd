-- =============================================================================
-- seed_phase1.sql — 【已废弃，请勿执行】
--
-- 此脚本为旧"新建数据库"方案设计（snake_case 表：users/patients/...），
-- 当前系统已完全切换为老血透 PostgreSQL（dialysis@10.10.8.83），
-- 所有数据直接读写 PascalCase 遗留表（Identity_Users/Register_PatientInfomation/...）。
-- 执行此脚本将因"relation does not exist"全部报错。
--
-- 登录账号：使用老血透系统 Identity_Users 表中 TenantId=3 的账号。
-- 备用密码：admin@123qwe（仅在 GIN_MODE!=release 时有效）。
-- =============================================================================

-- 以下语句均已失效，保留仅供历史参考，禁止在任何环境执行。
-- =============================================================================
-- seed_phase1.sql — 首轮环境初始化种子数据（历史，已废弃）
