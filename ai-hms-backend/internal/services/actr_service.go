...
func (s *ActrService) ensurePatientMapping(ctx context.Context, legacyPatientID int64) (int64, string, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "status 409") {
			if actrsID, ok := s.searchExistingPatient(ctx, dialysisNo, legacyPatientID); ok {
				return actrsID, dialysisNo, nil
			}
		}
...
func (s *ActrService) searchExistingPatient(ctx context.Context, dialysisNo string, legacyPatientID int64) (int64, bool) {
	results, err := s.client.SearchPatients(ctx, dialysisNo)
	if err != nil {
		return 0, false
	}
	for _, r := range results {
		if r.DialysisID == dialysisNo {
			m := &models.ExternalPatientMapping{
				ID:                utils.GenerateID(),
				TenantID:          s.tenantID,
				LegacyPatientID:   legacyPatientID,
				ExternalSystem:    ExternalSystemACTRS,
				ExternalPatientID: strconv.FormatInt(r.ID, 10),
				DialysisNo:        &dialysisNo,
				MatchStatus:       models.MatchStatusConfirmed,
			}
			if err := s.mapping.CreateMapping(m); err != nil {
				if strings.Contains(err.Error(), "duplicate") || strings.Contains(err.Error(), "unique") || errors.Is(err, gorm.ErrDuplicatedKey) {
					var fallback models.ExternalPatientMapping
					if e := s.db.Where("tenant_id = ? AND external_system = ? AND legacy_patient_id = ?",
						s.tenantID, ExternalSystemACTRS, legacyPatientID).First(&fallback).Error; e == nil {
						actrsID, _ := strconv.ParseInt(fallback.ExternalPatientID, 10, 64)
						return actrsID, true
					}
				}
...
func (s *ActrService) Status(ctx context.Context) ActrStatus {
	st := ActrStatus{
		Enabled:    s.enabled,
	...
	defer cancel()
	st.Reachable = s.client.Reachable(pingCtx)
	return st
}
...
func (s *ActrService) upsertPatientACTR(rec *models.PatientACTR) (*models.PatientACTR, error) {
	...
	updates := map[string]any{
		...
		"actr1": rec.ACTR1, "actr2": rec.ACTR2,
		"heart_width": rec.HeartWidth, "lung_width": rec.LungWidth, "tilt_angle": rec.TiltAngle,
	...
	if rec.DoctorCorrection != nil {
		updates["doctor_correction"] = rec.DoctorCorrection
	}
	if rec.Notes != "" {
	...
	if err := s.db.Where("tenant_id = ? AND patient_id = ? AND actrs_xray_id = ?",
		rec.TenantID, rec.PatientID, rec.ActrsXrayID).First(&existing).Error; err != nil {
		return nil, err
	}
	return &existing, nil
}
