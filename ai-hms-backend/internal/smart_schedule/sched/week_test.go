...
func TestIsDue(t *testing.T) {
	cases := []struct {
		...
		{FreqMonWedFri, day(2025, 1, 6), true},
		{FreqMonWedFri, day(2025, 1, 7), false},
		{FreqMonWedFri, day(2025, 1, 8), true},
		{FreqTueThuSat, day(2025, 1, 7), true},
		{FreqTwoPerWk, day(2025, 1, 9), true},
		{FreqTwoPerWk, day(2025, 1, 10), false},
		{FreqOnePerWk, day(2025, 1, 9), true},
		{FreqOnePerWk, day(2025, 1, 7), false},
		{FreqTemporary, day(2025, 1, 6), false},
	}
	for _, c := range cases {
...
func TestDecideMode(t *testing.T) {
	...
	if got := DecideMode(anchor, ModeHD, true, wd, par, day(2025, 1, 8)); got != ModeHDF {
		t.Errorf("偶周周三应为 HDF,得 %s", got)
	}
	if got := DecideMode(anchor, ModeHD, true, wd, par, day(2025, 1, 15)); got != ModeHD {
		t.Errorf("奇周周三应为 HD,得 %s", got)
	}
	if got := DecideMode(anchor, ModeHD, true, wd, par, day(2025, 1, 6)); got != ModeHD {
		t.Errorf("非 HDF 日应为 HD,得 %s", got)
	}
	if got := DecideMode(anchor, ModeHD, false, wd, par, day(2025, 1, 8)); got != ModeHD {
		t.Errorf("未启用 HDF 应为基础模式,得 %s", got)
	}
	if got := DecideMode(anchor, ModeHFD, true, wd, par, day(2025, 1, 6)); got != ModeHFD {
		t.Errorf("基础模式 HFD 非HDF日应为 HFD,得 %s", got)
	}
}
