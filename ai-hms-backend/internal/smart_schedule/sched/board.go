...
// WardOpenOn 某区在某日是否开放(假日值班模式下只开 OpenWardIds;正常日全开)。
// 决策 19:HolidayMode=20(值班模式)当天仍透析,但只开放部分区;空 OpenWardIds 视为全开。
func (b *Board) WardOpenOn(d time.Time, wardID int64) bool {
	c, ok := b.calendarByDate[dkey(d)]
	if !ok || c.HolidayMode != 20 {
		return true
	}
	if strings.TrimSpace(c.OpenWardIds) == "" {
		return true
	}
	for _, s := range strings.Split(c.OpenWardIds, ",") {
		if id, err := strconv.ParseInt(strings.TrimSpace(s), 10, 64); err == nil && id == wardID {
			return true
		}
	}
	return false
