		...
		for _, ward := range e.Board.wards {
			...
			for _, shift := range e.Board.shifts {
				...
				for _, it := range group[ws{ward.Id, shift.Id}] {
					...
					mode := DecideMode(e.Board.Anchor, it.DefaultMode, it.HdfEnabled, it.HdfWeekday, it.HdfWeekParity, d)
...
// placeCell 两轮分配(规范 §4.1,决策 4/24):先"需 HDF 机"的(HDF/HF)进 HDF 机,再 HD 类(HD/HFD/HP)。
func (e *Engine) placeCell(cell Cell, sessions []SessionItem) {
	// 第一轮:HDF/HF 必须上 HDF 机
	for _, s := range sessions {
		if !RequiresHdfMachine(s.Mode) {
			continue
		}
		...
		if m == nil {
			...
			e.draft(s.PatientId, cell, m, s.Mode, SourceRegular, &s.Item.Id, false)
		}
	}
	// 第二轮:HD 类(HD/HFD/HP)优先 HD 机,满了溢出 HDF 机
	for _, s := range sessions {
		if RequiresHdfMachine(s.Mode) {
			continue
		}
		m := e.placeHDSession(cell, s)
		if m != nil {
			e.draft(s.PatientId, cell, m, s.Mode, SourceRegular, &s.Item.Id, false)
			continue
		}
...
func (e *Engine) spillToLaterSlot(s SessionItem, cell Cell) *Cell {
	for _, cand := range e.laterSlots(cell) {
		...
		if m := e.placeHDSession(cand, s); m != nil {
			sug := e.draft(s.PatientId, cand, m, s.Mode, SourceRegular, &s.Item.Id, false)
